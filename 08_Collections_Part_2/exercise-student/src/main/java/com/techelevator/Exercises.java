...
public class Exercises {
	...
	public String animalGroupName(String animalName) {
		...
		if (herdName == null);
		return animalherds.get(animalName);
	}
	...
	public double isItOnSale(String itemNumber) {
		...
		Map<String, Double> discountsPercentage = new HashMap<String, Double>();
		discountsPercentage.put("bedroom3434", 0.60);
		discountsPercentage.put("kitchen4001", 0.20);
		if (discountsPercentage.containsKey(itemNumber)){
			return discountsPercentage.get(itemNumber);
		}
		return 0.0;
	}
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		...
		int peteMoney = peterPaul.get("Peter");
		int paulMoney = peterPaul.get("Paul");
		if (peteMoney > 0 && paulMoney < 1000){
			paulMoney += (peteMoney / 2);
			peteMoney = (peteMoney / 2) + (peteMoney % 2);
			peterPaul.put("Paul", paulMoney);
			peterPaul.put("Peter", peteMoney);
		}
		return peterPaul;
	...
	public Map<String, String> beginningAndEnding(String[] words) {
		Map<String, String> firstKeyAndLastCha = new HashMap<String, String>();
		for (int i= 0; i < words.length; i++) {
			String word = words[i];
			firstKeyAndLastCha.put(word.substring(0, 1), word.substring(word.length() - 1, word.length()));
		}
		return firstKeyAndLastCha;
	...
	public Map<Integer, Integer> integerCount(int[] ints) {
		...
		for (int integerCheck : ints) {
			if(integerCount.containsKey(integerCheck)) {
				int currentCount = integerCount.get(integerCheck);
				currentCount++;
				integerCount.put(integerCheck, currentCount) ;
			} else {
				integerCount.put(integerCheck, 1);
			}
		}
		return integerCount;
